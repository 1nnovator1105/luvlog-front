...
function Write(prop) {
  ...
  const IMAGE_RESIZER = (file, fileFormat, outputType) =>
    new Promise((resolve) => {
  ...
  const saveBtnClickListener = () => {
    ...
    const URL = "https://luvlog-server.vercel.app/api/write";
    const PARAMS = {
      title: title,
      author: "ysms",
      contents: getHtml,
  ...
  const uploadImage = async (blobFile, filename) => {
    ...
    const URL = "https://luvlog-server.vercel.app/api/upload";
    const PARAMS = {
      data: formData,
    };
    const HEADERS = {
    ...
    <>
      ...
      <div style={{ maxWidth: "1024px", margin: "20px auto 0" }}>
        <input
          ...
          type="text"
          placeholder="제목"
          onChange={tileBoxChangeListener}
          maxLength="80"
        />
        <Editor
          style={{ color: "unset" }}
        ...
        <button className="saveBtn" onClick={saveBtnClickListener}>
          글저장
        </button>
      </div>
    </>
